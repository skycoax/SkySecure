 ...
 *       override and double-extension tricks with zero bytes), and mark an
 *       installer red. The download itself is NOT started: fetching a file is
 *       the user's decision, taken by tapping it, and the full scan runs
 *       automatically the moment those tapped-for bytes land.</li>
 * </ul>
 *
    ...
    public static void ensure(int account, MessageObject message, File plannedFile) {
        ...
        AndroidUtilities.runOnUIThread(() -> {
            try {
                ...
                // Name-only preview, and nothing more. Reading the name costs
                // no network and no bytes — it is the check that catches the
                // right-to-left override — and it is also where this method now
                // stops. The download is the user's decision alone: it starts
                // when they tap the file, never on our initiative, and the full
                // scan follows automatically the moment those bytes land (the
                // FileLoader hook). `false` here means "no scan is on its
                // way", so the preview never parks a spinner on the bubble.
                gate.previewByName(displayName, plannedFile, message, false);
            } catch (Throwable t) {
